...
export class Embed {
  public static base(user?: User): RichEmbed {
    const embed = new RichEmbed()
      .setFooter(
        'ArctBot',
        'https://cdn.discordapp.com/avatars/508692053966848031/893e48c96ac55fe3980a671af388137c.webp?size=128'
      )
      .setTimestamp()

    return user ? embed.setAuthor(user.username, user.displayAvatarURL) : embed
  }

  public static info(user?: User): RichEmbed {
    return Embed.base(user).setColor(0x149ddf)
  }

  public static warn(user?: User): RichEmbed {
    return Embed.base(user).setColor(0xffb825)
  }
  ...
  public static log(
    ...
    return (level === 'info' ? this.info(user) : this.warn(user))
      .addField(
        'Log Entry',
        `Command $${command.name} has been executed by ${author}`
      )
  ...
  public static args(user?: User): RichEmbed {
    return Embed.error(
      'Invalid Arguments!\n Use $help <command> to find usage for this command.',
      user
    )
  }
}
