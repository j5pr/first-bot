...
export default new (class Math extends Command {
  ...
  public usage = 'math [scope] <expression>'
  ...
  public options = [
    ...
    },
    {
      ...flag,
      name: 'simplify',
      description: 'Use the simplify() function',
      alias: 's'
    },
    {
      ...flag,
      name: 'derivative',
      description: 'Use the derivative() function',
      alias: 'd'
    },
    {
      ...string,
      name: 'scope',
      description: 'Provide a scope',
      alias: 'x',
      default: '{}'
    }
  ]
  ...
  public async run(
    ...
    try {
      if (args.evaluate) {
        await message.channel.send(
          `\`\`\`${expr} = ${math.evaluate(expr, scope)}\`\`\``
        )
      } else if (args.simplify) {
        await message.channel.send(
          `\`\`\`simplify(${expr}) = ${math.simplify(expr)}\`\`\``
        )
      } else if (args.derivative) {
        await message.channel.send(
          `\`\`\`derivative(${expr}) = ${math.derivative(
            expr,
            scope.v || 'x'
          )}\`\`\``
        )
        ...
        await message.channel.send({
          embed: Embed.error('Invalid operation!', message.author)
        })
      }
    } catch (e) {
      await message.channel.send({
